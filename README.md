...
Локальный прототип AI-агента для анализа текстовых обращений пользователей. Проект сделан как небольшое тестовое задание: его можно быстро установить, запустить без внешних ключей и проверить через API, CLI и тесты.

## Выбранный кейс

Кейс: первичная обработка обращений в поддержку. Агент принимает текст, нормализует его, определяет тип обращения, кратко суммирует проблему, выставляет приоритет и решает, нужна ли передача человеку.
...
- Хранение обращений в PostgreSQL.
...
- Human-in-the-loop процесс.
- Мониторинг качества классификации.
- Версионирование промптов.
- Batch-обработка.
- Подключение CRM или helpdesk.

## Готовность к публикации

Проект не содержит секретов, виртуального окружения или архивов. Он готов к загрузке в публичный GitHub-репозиторий после обычной проверки владельцем: `ruff check .` и `pytest`.
